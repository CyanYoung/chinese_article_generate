...
def split(rate, align_seqs, align_inds, vocab_num):
    bound = int(len(align_seqs) * rate)
    seq_train, ind_train = align_seqs[:bound], align_inds[:bound]
    x_dev = np.array(align_seqs[bound:])
    y_dev = to_categorical(align_inds[bound:], num_classes=vocab_num)
...
def fit(name, epoch, embed_mat, align_seqs, align_inds, step):
    vocab_num, embed_len = embed_mat.shape
    ...
    seq_train, ind_train, x_dev, y_dev = split(0.9, align_seqs, align_inds, vocab_num)
    for l_rate in np.arange(0, 1, step):
        x_train, y_train = get_part(l_rate, step, seq_train, ind_train, vocab_num)
...
if __name__ == '__main__':
    fit('rnn_plain', 10, embed_mat, align_seqs, align_inds, step=0.2)
    fit('rnn_stack', 10, embed_mat, align_seqs, align_inds, step=0.2)
